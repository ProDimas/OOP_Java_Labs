...
class LabDependencyGraph {
    ...
    <T> void satisfyDependentOn(ComponentNode<T> node) {
        this.listOfNodes.stream()
                .filter((n) -> n.isDependentOn(node.getType()))
                .forEach((n) -> n.satisfyDependency(node));
    }
    ...
    <T> List<ComponentNode<? extends T>> getNodesOfExtendingClasses(Class<T> clazz) {
        ...
        for (ComponentNode<?> node : this.listOfNodes) {
            if (clazz.isAssignableFrom(node.getType()) && !clazz.equals(node.getType())) {
    ...
    void checkOnCyclicDependencies() {
        Set<ComponentNode<?>> toInspect;

        for (ComponentNode<?> node : this.listOfNodes) {
            toInspect = node.getSatisfiedDependencies();
    ...
    <T> ComponentNode<T> searchForComponent(Class<T> clazz) {
        return this.listOfNodes.stream()
                ...
                .findFirst()
                .orElseThrow(() -> {
            throw new NoSuchElementException(clazz.getName().concat(": has not been registered in container"));
        });
    }
}
